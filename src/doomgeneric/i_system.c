...
// Text of the last I_Error, read by the platform layer on the way out.
char i_error_msg[256];
...
void I_Error (char *error, ...)
{
    ...
    // The stderr copy above goes to a UDP log nobody watching the TV can see,
    // so without this an I_Error is indistinguishable from a lock-up.
    M_StringCopy(i_error_msg, msgbuf, sizeof(i_error_msg));
