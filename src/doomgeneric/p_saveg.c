...
char *P_TempSaveGameFile(void)
{
    ...
    if (filename == NULL)
    {
        // Scratch storage, not savegamedir. savegamedir is inside the savedata
        // container that also holds the PS2 memory cards, and it is read-only
        // except during the brief copy at the end of G_DoSaveGame; building
        // the file here keeps that window to one copy.
        filename = M_StringJoin(DG_SAVE_SCRATCH, "temp.dsg", NULL);
    }
...
void P_ArchiveThinkers (void)
{
    ...
    {
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(P_MobjThinker))
	{
...
void P_UnArchiveThinkers (void)
{
	...
	if (currentthinker->function.acp1 == (actionf_p1)FORCE_FUNCPTR(P_MobjThinker))
	...
	switch (tclass)
	{
	  ...
	  case tc_mobj:
	    ...
	    mobj->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(P_MobjThinker);
...
void P_ArchiveSpecials (void)
{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_MoveCeiling))
	{
            saveg_write8(tc_ceiling);
	    saveg_write_pad();
            saveg_write_ceiling_t((ceiling_t *) th);
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_VerticalDoor))
	{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_MoveFloor))
	{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_PlatRaise))
	{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_LightFlash))
	{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_StrobeFlash))
	{
	...
	if (th->function.acp1 == (actionf_p1)FORCE_FUNCPTR(T_Glow))
	{
...
void P_UnArchiveSpecials (void)
{
	...
	switch (tclass)
	{
	  ...
	  case tc_ceiling:
		...
		ceiling->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_MoveCeiling);
	  ...
	  case tc_door:
	    ...
	    door->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_VerticalDoor);
	  ...
	  case tc_floor:
	    ...
	    floor->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_MoveFloor);
	  ...
	  case tc_plat:
		...
		plat->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_PlatRaise);
	  ...
	  case tc_flash:
	    ...
	    flash->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_LightFlash);
	  ...
	  case tc_strobe:
	    ...
	    strobe->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_StrobeFlash);
	  ...
	  case tc_glow:
	    ...
	    glow->thinker.function.acp1 = (actionf_p1)FORCE_FUNCPTR(T_Glow);
