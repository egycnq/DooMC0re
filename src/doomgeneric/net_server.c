...
static void NET_SV_BroadcastMessage(char *s, ...)
{
    ...
    for (i=0; i<MAXNETNODES; ++i)
    {
        ...
        {
            // buf already holds the formatted text, and it can contain a
            // player name that came off the wire -- passing it as the format
            // string hands a stranger the % directives.
                            ...
                            net_addr_t *addr)
{
    ...
    // Most of the drops below send no reject packet, so the client just sees
    // silence -- the log is the only place they show up at all.
    if (!NET_ReadInt32(packet, &magic))
    {
        printf("SV: %s: SYN too short to hold a magic number\n",
    ...
    switch (magic)
    {
        ...
        default:
            printf("SV: %s: SYN with magic %08x, not ours -- ignored\n",
                   NET_AddrToString(addr), magic);
            return;
    }
    ...
    {
        printf("SV: %s: SYN has no version string\n", NET_AddrToString(addr));
        return;
    }
    ...
    // and the max_players value is in a sensible range. Split out from the one
    // condition it used to be so the log names the field that was wrong.
    if (!NET_ReadConnectData(packet, &data))
    {
        printf("SV: %s: SYN has malformed connect data\n",
               NET_AddrToString(addr));
        return;
    }
    ...
    {
        printf("SV: %s: bad game mode %i / mission %i\n",
               NET_AddrToString(addr), data.gamemode, data.gamemission);
        return;
    }

    if (data.max_players > NET_MAXPLAYERS)
    {
        printf("SV: %s: asked for %i players, %i is the limit\n",
               NET_AddrToString(addr), data.max_players, NET_MAXPLAYERS);
        return;
    }
    ...
    if (player_name == NULL)
    {
        printf("SV: %s: SYN has no player name\n", NET_AddrToString(addr));
        return;
    }
...
// Names the first setting NET_ValidGameSettings would object to. It only
// returns a boolean, and "the lobby never starts" is not enough to go on when
// the two machines are in different houses. Mirrors net_common.c -- if the
// checks there change, change these.

static char *BadGameSetting(net_gamesettings_t *settings)
{
    ...
    if (settings->skill < sk_noitems || settings->skill > sk_nightmare)
        return "skill";
    if (!D_ValidGameVersion(sv_gamemission, settings->gameversion))
        return "gameversion";
    if (!D_ValidEpisodeMap(sv_gamemission, sv_gamemode, settings->episode,
                           settings->map))
        return "episode/map";

    return "nothing (checks have drifted apart)";
}
...
static void NET_SV_ParseGameStart(net_packet_t *packet, net_client_t *client)
{
    ...
    if (client == NET_SV_Controller())
    {
        if (!NET_ReadSettings(packet, &settings))
        {
            ...
            printf("SV: malformed game start packet from the controller\n");
            return;
        }
        ...
        {
            printf("SV: refusing game start, bad %s "
                   "(ticdup %i, extratics %i, dm %i, skill %i, version %i, "
                   "episode %i, map %i)\n",
                   BadGameSetting(&settings), settings.ticdup,
                   settings.extratics, settings.deathmatch, settings.skill,
                   settings.gameversion, settings.episode, settings.map);
            return;
        }
...
static void NET_SV_Packet(net_packet_t *packet, net_addr_t *addr)
{
    ...
    {
        // no packet type. Still has to reach the free below: a datagram too
        // short to hold one already cost us an address-table entry, and
        // returning here leaked it for every source port that sent one.
...
void NET_SV_CheckDeadlock(net_client_t *client)
{
    ...
    if (nowtime - client->last_gamedata_time > 1000)
    {
        ...
        // recvwindow is [BACKUPTICS][NET_MAXPLAYERS]; upstream has the
        // subscripts the other way round here, so this walked tics 0..15 of
        // every player interleaved and asked to resend a range it had not
        // actually missed. Diverges from Chocolate Doom 3.0.0 on purpose.
...
void NET_SV_Shutdown(void)
{
    ...
    while (running)
    {
        ...
        // Timed out? Was 5000, shortened for the same reason as the loop in
        // NET_CL_Disconnect. A peer on a bad link may now miss the disconnect
        // and wait out its own timeout, which beats a screen that looks dead.
