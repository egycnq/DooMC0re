...
// Why the server turned us away, in its own words. Empty if it did not, or if
// the connection failed some other way. The printf that goes with it lands in
// a UDP log nobody at the TV can read, so this ends up on the error screen.
extern char net_reject_reason[128];
