...
// The net code came over from Chocolate Doom, which binds per type. Our
// M_BindVariable takes a void *, so all this adds is the check that would
// otherwise be missing.

void M_BindStringVariable(char *name, char **location)
{
    if (GetDefaultForName(name)->type != DEFAULT_STRING)
    {
        I_Error("M_BindStringVariable: '%s' is not a string", name);
    }

    M_BindVariable(name, location);
}
...
char *M_GetSaveGameDir(char *iwadname)
{
    ...
    else
    {
        ...
        free(topdir);
#else
        // Per-IWAD, as the ORIGCODE path above does. One shared directory put
        // Doom 1 saves in Doom 2's load menu, and loading one crashed: the map
        // it names does not exist in the loaded WAD. iwadname comes from
        // D_SaveGameIWADName, which keys on gamemission rather than the
        // filename, so doom.wad and doom1.wad still share.
        savegamedir = M_StringJoin(configdir, DIR_SEPARATOR_S, ".savegame",
                                   DIR_SEPARATOR_S, iwadname,
                                   DIR_SEPARATOR_S, NULL);

        // The savedata mount is read-only outside a write transaction, so this
        // cannot create it here; DG_SaveBegin makes the whole chain.
        DG_SaveSetDir(savegamedir);
