...
//
// Upstream slept 100ms a pass and left the screen alone, which it could afford
// with a textscreen lobby on show. dg_net_wait waits by presenting instead, so
// the panel keeps ticking at 60Hz and Circle still gets read.
//
                            ...
                            netgame_startup_callback_t callback)
{
    ...
    while (!NET_CL_GetSettings(settings))
    {
        ...
        M_snprintf(line, sizeof(line), "%d OF %d READY",
                   net_client_wait_data.ready_players,
                   net_client_wait_data.num_players);

        if (dg_net_wait("STARTING GAME", line, "") == DG_NET_CANCEL)
        {
...
boolean D_InitNetGame(net_connect_data_t *connect_data)
{
     ...
     || M_CheckParm("-privateserver") > 0)
    {
        ...
        NET_SV_AddModule(&net_ps5_module);
    ...
    else
    {
        ...
        if (i > 0)
        {
            net_ps5_module.InitClient();
            addr = net_ps5_module.ResolveAddress(myargv[i+1]);
    ...
    if (addr != NULL)
    {
        ...
        // Cleared per attempt, or a timeout would report the refusal from an
        // earlier one and blame the wrong thing.
        net_reject_reason[0] = '\0';

        if (!NET_CL_Connect(addr, connect_data))
        {
            // A refusal and a silent timeout look the same on screen
            // otherwise, and they need opposite things done about them.
            if (net_reject_reason[0])
            {
                I_Error("%s said no: %s", NET_AddrToString(addr),
                        net_reject_reason);
            }

            I_Error("No answer from %s. Check the address, and that the "
                    "other end is waiting in its lobby.",
...
void TryRunTics (void)
{
	...
	if (I_GetTime() / ticdup - entertic > 0)
	{
        ...
        // Waiting on a peer is the one place a frame can run long, and the
        // mixer only produces when it is called. Without this the ring drains
        // for as long as we sit here and the player hears it.
        I_UpdateSound();
