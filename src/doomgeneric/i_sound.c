...
#endif /* FEATURE_SOUND */
#ifdef FEATURE_DG_SOUND
    /* Select by device like the SFX path does, rather than assigning blindly, so
       an unsupported snd_musicdevice leaves music_module NULL instead of half
       initialised. */
    if (SndDeviceInList(snd_musicdevice,
                        DG_music_module.sound_devices,
                        DG_music_module.num_sound_devices))
    {
        if (DG_music_module.Init())
        {
            music_module = &DG_music_module;
        }
    }
