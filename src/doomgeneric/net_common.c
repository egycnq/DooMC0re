                          ...
                          net_protocol_t protocol)
{
    ...
    // Without this the stamp starts at zero and NET_Conn_Run drops the
    // connection the moment the host has been up for CONNECTION_TIMEOUT_LEN.
    // Upstream chocolate-doom 5de10e5, which landed after the 3.0.x tags.
...
static void NET_Conn_ParseReject(net_connection_t *conn, net_packet_t *packet)
{
    ...
    if (conn->state == NET_CONN_STATE_CONNECTING)
    {
        ...
        // Kept for the error screen; see net_common.h.
        M_StringCopy(net_reject_reason, msg, sizeof(net_reject_reason));
