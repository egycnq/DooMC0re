...
// 8 rather than upstream's 16: net_server.c's clients[] costs 22 KB a node and
// the payload has to fit a fixed mapping. This is a local capacity limit only,
// nothing on the wire is sized by it, so a stock chocolate-doom still talks to
// us -- the ninth connection just gets "Server is full!".
