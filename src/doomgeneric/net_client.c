...
// Round trip from sending a ticcmd to the tic holding it coming back. Nothing
// is predicted locally, so this is also the delay the player feels, which is
// what the HUD readout is for.

int NET_CL_AverageLatency(void)
{
    return average_latency / FRACUNIT;
}
...
void NET_CL_Disconnect(void)
{
        ...
        && client_connection.state != NET_CONN_STATE_DISCONNECTED_SLEEP)
    {
        ...
        {
            // Was 5 seconds. Nothing is presented while we spin here and
            // NET_SV_Shutdown spins again straight after, so a quit on the
            // error path sat on a frozen screen for ten seconds before the
            // error appeared. Retries are one a second, so this is two of five.
